...
def test_simple_type() -> None:
    assert TyChecker.extract_revealed_type("Revealed type: `int`") == "int"


def test_literal_type() -> None:
    assert (
        TyChecker.extract_revealed_type("Revealed type: `Literal[1]`") == "Literal[1]"
    )


def test_string_literal() -> None:
    assert (
        TyChecker.extract_revealed_type('Revealed type: `Literal["a"]`')
        == 'Literal["a"]'
    )
